...
def parse_script(area):
    script_lines = []
    with open(f"{area}/scripts.inc") as f:
        script_lines = f.read().split('\n')

    with open(f"{area}/../../scripts/trainers.inc") as f:
        script_lines += f.read().split('\n')
...
def output_tracker(map_data, out):
    """Output the routeData table body for direct insertion into TrainerMapData.lua."""
    out.write("\t-- Generated by parse-trainerdata.py\n")
