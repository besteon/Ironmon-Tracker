...
RandomizerLog.Sectors = {
	-- Note, note all sectors start with "--", especially if the data is unchanged
	NewSectorStart = "--",
	-- First three lines: (if display this, also show game being played (1.0/1.1) and attempts #)
	--   Randomizer Version: 4.5.1
	--   Random Seed: 254941433205620
	--   Settings String: 320WRIBEjIBAAQABwCRAAKeBnsECQEAFAACCQ8uEgAAAAAABRi45ATkAYYICTIGBAIyAAUAEEZpcmUgUmVkIChVKSAxLjFebw3J48M4ig==
	Evolutions = {
		Head = "--Randomized Evolutions--",
		-- Matches: pokemon, evos
		Pattern = "^" .. RandomizerLog.Patterns.PokemonName .. "%s*%->%s*(.*)",
	},
	BaseStatsItems = { -- Currently unused, Get Base Stats from Movesets instead
		...
		Pattern = "^.*|" .. RandomizerLog.Patterns.PokemonName .. "%s*|.*|%s*(%d*)|%s*(%d*)|%s*(%d*)|%s*(%d*)|%s*(%d*)|%s*(%d*)|.*|.*|(.*)",
	},
	MoveSets = {
...
function RandomizerLog.parseLog(filepath)
	...
	Utils.printDebug("Total: %s", #logLines)
	RandomizerLog.parseEvolutions(logLines)
	Utils.printDebug(" Evos: %s", #logLines)
	RandomizerLog.parseBaseStatsItems(logLines)
	Utils.printDebug(" BST: %s", #logLines)
	RandomizerLog.parseMoveSets(logLines)
	Utils.printDebug(" Moves: %s", #logLines)
	RandomizerLog.parseTMMoves(logLines)
	Utils.printDebug(" TM Moves: %s", #logLines)
	RandomizerLog.parseTMCompatibility(logLines)
	Utils.printDebug(" TM Compatibility: %s", #logLines)
	RandomizerLog.parseTrainers(logLines)
	Utils.printDebug(" Trainers: %s", #logLines)
	RandomizerLog.parsePickupItems(logLines)
	Utils.printDebug(" Pickup: %s", #logLines)
	return true
end
...
function RandomizerLog.seekSectorStart(logLines, sectorHead)
	...
	while line ~= nil do
		if string.find(line, sectorHead, 1, true) ~= nil then
			return true
		end
...
function RandomizerLog.parseEvolutions(logLines)
	if not RandomizerLog.seekSectorStart(logLines, RandomizerLog.Sectors.Evolutions.Head) then
		return
	end
	...
	local line = table.remove(logLines, 1)
	while line ~= nil do
		local pokemon, evos = string.match(line, RandomizerLog.Sectors.Evolutions.Pattern)
		...
		RandomizerLog.Data[pokemonId].Evolutions = {}
		...
		for evo in string.gmatch(evos, RandomizerLog.Patterns.PokemonName) do
			...
			if RandomizerLog.PokemonNameToIdMap[evo] ~= nil then
				table.insert(RandomizerLog.Data[pokemonId].Evolutions, RandomizerLog.PokemonNameToIdMap[evo])
			end
		end
		line = table.remove(logLines, 1)
	end
...
function RandomizerLog.parseBaseStatsItems(logLines)
	if not RandomizerLog.seekSectorStart(logLines, RandomizerLog.Sectors.BaseStatsItems.Head) then
		return
	end
	...
	local line = table.remove(logLines, 1)
	while line ~= nil do
		local pokemon, hp, atk, def, spatk, spdef, spd, helditems = string.match(line, RandomizerLog.Sectors.BaseStatsItems.Pattern)
		pokemon = RandomizerLog.formatInput(pokemon)
		...
		local pokemonId = RandomizerLog.PokemonNameToIdMap[pokemon]
		local pokemonData = RandomizerLog.Data[pokemonId]
...
function RandomizerLog.parseMoveSets(logLines)
	if not RandomizerLog.seekSectorStart(logLines, RandomizerLog.Sectors.MoveSets.Head) then
		return
	end
	...
	while line ~= nil do
		...
		if pokemonData ~= nil then
			...
			line = table.remove(logLines, 1)
			local level, movename = string.match(line, RandomizerLog.Sectors.MoveSets.Pattern)
			while level ~= nil and movename ~= nil do
				...
				table.insert(RandomizerLog.Data[pokemonId].MoveSet, nextMove)

				line = table.remove(logLines, 1)
				if line == nil then
					return
				end
				level, movename = string.match(line, RandomizerLog.Sectors.MoveSets.Pattern)
			end
		else
			line = table.remove(logLines, 1)
		end
...
function RandomizerLog.parseTMMoves(logLines)
	if not RandomizerLog.seekSectorStart(logLines, RandomizerLog.Sectors.TMMoves.Head) then
		return
	end
...
function RandomizerLog.parseTMCompatibility(logLines)
	-- Utils.printDebug("[%s] \"%s\" = %s, %s", i, logLines[i] or "N/A", level or "0", movename or "N/A")
