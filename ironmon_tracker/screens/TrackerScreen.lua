	...
	for _, statKey in ipairs(Constants.OrderedLists.STATSTAGES) do
		TrackerScreen.Buttons[statKey] = {
			...
			onClick = function(self)
				...
				local pokemon = Tracker.getPokemon(Utils.inlineIf(Tracker.Data.isViewingLeft or Tracker.Data.isViewingOwn, Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_LEFT], Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_RIGHT]), false)
				if pokemon ~= nil then
...
function TrackerScreen.updateButtonStates()
	local opposingPokemon = Tracker.getPokemon(Utils.inlineIf(Tracker.Data.isViewingLeft or Tracker.Data.isViewingOwn, Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_LEFT], Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_RIGHT]), false)
...
function TrackerScreen.drawScreen()
	...
	if Tracker.Data.isViewingOwn then
		viewedPokemon = Tracker.getPokemon(Utils.inlineIf(Tracker.Data.isViewingLeft, Battle.ViewSlots[Battle.BATTLE_INDEXES.OWN_VIEWSLOT_LEFT],Battle.ViewSlots[Battle.BATTLE_INDEXES.OWN_VIEWSLOT_RIGHT]), true)
		opposingPokemon = Tracker.getPokemon(Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_LEFT], false)
	else
		viewedPokemon = Tracker.getPokemon(Utils.inlineIf(Tracker.Data.isViewingLeft, Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_LEFT],Battle.ViewSlots[Battle.BATTLE_INDEXES.OTHER_VIEWSLOT_RIGHT]), false)
